# Анализатор логов ADS-B и Mode S (v2)
...
При запуске `main.py` программа обрабатывает файл логов (автоматически ищет в папке `data` или принимает путь через аргументы). В консоль выводится таблица со следующими данными:
...
* **Флаги данных** (наличие (+) или отсутствие (-)):
    ...
    * **NB (NIC Baro)** — флаг контроля целостности барометрической высоты (0 или 1), указывающий на проверку высоты бортовыми системами на достоверность.
...
1.  **ОБЩАЯ КАРТА (All Tracks)** — траектории всех обнаруженных бортов на одной карте.
...
6.  **Координаты** — график изменения широты от времени.
7.  **Курс** — график изменения курса (Track/Heading) с цветовой индикацией качества NIC.
...
11. **Диагностика целостности (Integrity)** — Графики NIC (Nav Integrity Category) и SIL (Source Integrity Level).
12. **Диагностика точности (Accuracy)** — Графики NACp (Position Accuracy), GVA (Vertical Accuracy) и NACv (Velocity Accuracy).
13. **HIL (Horizontal Integrity Limit)** — график расчетного горизонтального предела целостности в метрах (на основе NIC). Показывает максимально допустимую ошибку позиционирования.
14. **HFOM (Horizontal Figure of Merit)** — горизонтальная точность определения координат в метрах с вероятностью 95% (на основе NACp).
15. **VFOM (Vertical Figure of Merit)** — вертикальная точность определения высоты в метрах (на основе GVA).
16. **Сводный график качества (%)** — интегральный график, переводящий значения HIL, HFOM и VFOM в проценты (0-100%) для наглядной оценки стабильности сигнала. Содержит справочную плашку с расшифровкой метрик.
...
1.  **Структура папок:**
    Убедитесь, что все файлы программы (`main.py`, `decoder.py`, `visual.py` и т.д.) находятся в одной директории.
    
2.  **Подготовка данных:**
    Создайте папку `data` рядом с `main.py` и положите туда файлы логов (расширение `.t4433`, настраивается в `config.py`).

3.  **Запуск программы:**

    * **Автоматический режим:**
        Программа найдет первый файл `.t4433` в папке `data` и обработает его.
        ```bash
        python3 main.py
        ```

    * **Указание конкретного файла:**
        Можно указать путь к любому файлу логов.
        ```bash
        python3 main.py -f "путь/к/файлу.log"
        ```

    * **Фильтрация по одному борту (ICAO):**
        Загрузит файл, но построит графики и выведет детальную информацию только для указанного hex-адреса.
        ```bash
        python3 main.py -a 4249F5
        ```